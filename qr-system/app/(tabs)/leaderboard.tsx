  ...
  const loadStudentData = async (url?: string) => {
    ...
    try {
      ...
      if (
        ...
        response = await studentApi.getLeaderboardByGradeAndSection(
          selectedGrade,
          selectedSection,
          url
        );
        ...
        response = await studentApi.getLeaderboardByGrade(selectedGrade, url);
      } else {
        // No filter, get all leaderboard data
        response = await studentApi.getLeaderboard(url);
      }
      ...
      if (isMountedRef.current) {
        console.log(
          `Loaded ${studentsArray.length} students from API leaderboard`
        );
          ...
          <View className="flex-1">
            ...
            >
              ...
              <Ionicons
                name="chevron-down"
                size={16}
                color={selectedGrade === t("allGrades") ? mutedColor : mutedColor}
              />
            </TouchableOpacity>
          </View>
        </View>
