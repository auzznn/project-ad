...
export default function KehadiranPage() {
  ...
  const fetchAttendance = async (pageNumber: number = 1) => {
    setLoading(true);
    try {
      let url = `http://localhost:8080/api/student_attendance/daily/?page=${pageNumber}`;

      if (gradeFilter !== "" && classFilter !== "") {
        url = `http://localhost:8080/api/student_attendance/daily/${gradeFilter}/${classFilter}/?page=${pageNumber}`;
      }
